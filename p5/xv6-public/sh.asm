     ...
     efc:	89 d1                	mov    %edx,%ecx
{
  ...
  do{
    buf[i++] = digits[x % base];
     ...
     f29:	0f b6 92 48 21 00 00 	movzbl 0x2148(%edx),%edx
     ...
     fa9:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    1019:	ff 24 85 f0 20 00 00 	jmp    *0x20f0(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
  ...
  write(fd, &c, 1);
    ...
    1127:	90                   	nop
          s = "(null)";
    1128:	ba e8 20 00 00       	mov    $0x20e8,%edx
        while(*s != 0){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1141:	a1 84 40 00 00       	mov    0x4084,%eax
{
  ...
  freep = p;
    ...
    1181:	5b                   	pop    %ebx
  freep = p;
    1182:	89 15 84 40 00 00    	mov    %edx,0x4084
}
    1188:	5e                   	pop    %esi
    1189:	5f                   	pop    %edi
    118a:	5d                   	pop    %ebp
  ...
  freep = p;
    11bc:	89 15 84 40 00 00    	mov    %edx,0x4084
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    11dc:	8b 3d 84 40 00 00    	mov    0x4084,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    1221:	8b 3d 84 40 00 00    	mov    0x4084,%edi
  ...
  return freep;
    124f:	8b 15 84 40 00 00    	mov    0x4084,%edx
    ...
    127c:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
    127f:	89 15 84 40 00 00    	mov    %edx,0x4084
}
    ...
    128f:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
    1290:	c7 05 84 40 00 00 88 	movl   $0x4088,0x4084
    1297:	40 00 00 
    base.s.size = 0;
    129a:	bf 88 40 00 00       	mov    $0x4088,%edi
    base.s.ptr = freep = prevp = &base;
    129f:	c7 05 88 40 00 00 88 	movl   $0x4088,0x4088
    12a6:	40 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    12a9:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
    12ab:	c7 05 8c 40 00 00 00 	movl   $0x0,0x408c
    12b2:	00 00 00 
    if(p->s.size >= nunits){
